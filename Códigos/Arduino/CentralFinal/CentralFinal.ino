...
{  
  for (auto shield : vecMyoWareShields)
  {
    ...
    sensor = myArray[0]; // Obtener valor de sensor fuera de array recibido
    valMapeado = map(sensor, 0, 550, 190, 127); //Mapear valor en escala promediada
    myArray[0] = valMapeado;
    
    for (int i = 0; i < 2; i++) {
      Serial.print(myArray[i]);
      if (i < 1) Serial.print(", "); //Enviar Array a MATLAB
    }
  }
  Serial.println("");
}
