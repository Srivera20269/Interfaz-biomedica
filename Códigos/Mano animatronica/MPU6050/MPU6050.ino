...
//Gyro Variables
float elapsedTime, ptime, timePrev;        //Variables for time control
...
void setup() {
  ...
  ptime = millis();                        //Start counting time in milliseconds
 ...
 * I make the mean of 200 values, that should be enough*/
  if(gyro_error==0)
  {
    for(int i=0; i<200; i++)
    {
      ...
      Wire.endTransmission(false);
      Wire.requestFrom(MPU_addr,4,true);           //We ask for just 4 registers 
...
void loop() {
  timePrev = ptime;                        // the previous time is stored before the actual time read
  ptime = millis();                        // actual time read
  elapsedTime = (ptime - timePrev) / 1000; //divide by 1000 in order to obtain seconds
  ...
  Wire.requestFrom(MPU_addr,6,true);    //We ask for next 6 registers starting withj the 3B  
 ...
 * I only print the Y angle value for this test */
 ...
 if (Total_angle_y > 0)
 {
   probemos = map(Total_angle_y, 0, 90, 0, 63);
 }
 else
 {
    probemos = map(Total_angle_y, -90, 0, 0, -43);
 }
